...
async def start_check(project):
    ...
    if len(p) > 0:
        ...
        if n.add_nodes(nodes, username, password, project, False) == False:
            logger("VM Check couldn't start because inventory was not created","error")
        else:
            provider = p[0]['provider']
            print(provider)
            playbook = "compatibility.yaml"
            stage = "vm_check"
            curr_dir = os.getcwd()
            print(curr_dir)
            ...
            try:
                if run_playbook:
                    print("****************VM Check completed*****************")
                    logger("VM Check completed", "info")
                                        
                    hosts = BluePrint.objects(project=project).allow_filtering()
                    for host in hosts:
                        BluePrint.objects(host=host.host, project=project).update(status="25")
                else:
                    print("VM Check failed")
                    logger("VM Check failed", "error")
            except Exception as e:
                print(str(e))
            finally:
                con.shutdown()    